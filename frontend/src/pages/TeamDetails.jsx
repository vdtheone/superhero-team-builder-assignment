            ...
            members.map(hero => (
              <div key={hero.id} className="border rounded-lg p-3 flex items-center justify-between bg-gray-50 hover:border-blue-300 transition-colors">
                <div className="flex items-center gap-3 overflow-hidden">
                  {hero.image_url ? (
                    <img src={`http://localhost:8001/api/heroes/image-proxy/?url=${hero.image_url}`} alt={hero.name} className="w-12 h-12 rounded-full object-cover bg-gray-200 shrink-0" />
                  ) : (
